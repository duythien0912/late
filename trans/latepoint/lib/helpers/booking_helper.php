      ...
      <div class="os-current-month-label">
        <div class="current-month">
          <?php if($settings['highlight_target_date']){
            echo OsTimeHelper::get_nice_date_with_optional_year($target_date->format('M d - Y'), false);
          }else{
  ...
  return true;
          ...
          function onClickExportPdf(){
              console.log("onClickExportPdf");
                ...
                var bodyHtml = "<h1 style='color: #1C878E'>Insert here data of day</h1>";
                jQuery(".ch-day-booking-i").each(function () {
                    bodyHtml +=  "<span style='color: #1C878E'> #"+ jQuery(this).attr("data-id") +" </span>"
                    bodyHtml +=  "- <span> "+ jQuery(this).attr("data-service-name") +" </span>"
                    bodyHtml +=  "- <span> "+ jQuery(this).attr("data-agent-fullname") +" </span>"
                    bodyHtml +=  "- <span> "+ jQuery(this).attr("data-service-time") +" </span>"
                    bodyHtml +=  "<br/>"
                });
            ...
            function onClickExportCSV() {
                console.log("onClickExportCSV");
                var rows = [];
                jQuery(".ch-day-booking-i").each(function () {
                    var data = [];
                    data.push(jQuery(this).attr("data-id"));
                    data.push(jQuery(this).attr("data-service-name"));
                    data.push(jQuery(this).attr("data-agent-fullname"));
                    data.push(jQuery(this).attr("data-service-time"));
                    rows.push(data);
                });
...
today = dd + '/' + mm + '/' + yyyy;
        ...
        link.setAttribute("download", "Appointment-"+ today+ ".csv");
      ...
      <div class="os-month-control-buttons-w">
        ...
        <?php if($settings['layout'] == 'horizontal') echo '<button onclick="onClickExportPdf()" class="os-month-today-pdf os-month-today-btn">'.__('Export PDF', 'latepoint').'</button>'; ?>
        <?php if($settings['layout'] == 'horizontal') echo '<button onclick="onClickExportCSV()" class="os-month-today-export os-month-today-btn">'.__('Export CSV', 'latepoint').'</button>'; ?>
  ...
  public static function get_statuses_list(){
    return array( LATEPOINT_BOOKING_STATUS_APPROVED => __('Approvato', 'latepoint'), 
                  LATEPOINT_BOOKING_STATUS_PENDING => __('Da Approvare', 'latepoint'), 
                //   LATEPOINT_BOOKING_STATUS_PAYMENT_PENDING => __('Payment Pending', 'latepoint'), 
                  LATEPOINT_BOOKING_STATUS_CANCELLED => __('Cancellato', 'latepoint'));
  }
  ...
  public static function get_weekdays_arr($full_name = false) {
    if($full_name){
      $weekdays = array(__('lunedì', 'latepoint'), 
                        __('martedí', 'latepoint'), 
                        __('mercoledì', 'latepoint'), 
                        __('giovedì', 'latepoint'), 
                        __('venerdì', 'latepoint'), 
                        __('sabato', 'latepoint'), 
                        __('domenica', 'latepoint'));
    }else{
