<div class="os-form-w">
	<form action="" data-os-output-target=".os-wizard-step-content-i" data-os-after-call="latepoint_wizard_item_editing_cancelled" data-os-action="<?php echo OsRouterHelper::build_route_name('wizard', 'save_agent'); ?>">
		<div class="os-row">
			<div class="os-col-6">
		    <?php echo OsFormHelper::text_field('agent[first_name]', __('Nome ', 'latepoint'), $agent->first_name); ?>
			</div>
			<div class="os-col-6">
		    <?php echo OsFormHelper::text_field('agent[last_name]', __('Cognome', 'latepoint'), $agent->last_name); ?>
			</div>
		</div>
		<div class="os-row">
			<div class="os-col-lg-6">
		    <?php echo OsFormHelper::text_field('agent[email]', __('Indirizzo Email', 'latepoint'), $agent->email); ?>
			</div>
			<div class="os-col-lg-6">
		    <?php echo OsFormHelper::text_field('agent[phone]', __('Il tuo Numero di Telefono', 'latepoint'), $agent->phone); ?>
			</div>
		</div>
    ...
    <div class="side-by-side-buttons">
    	<div class="os-row">
    		<?php if(!isset($hide_cancel_btn)){ ?>
					<div class="os-col-lg-6">
			      <button type="button" data-os-after-call="latepoint_wizard_item_editing_cancelled" data-os-output-target=".os-wizard-step-content" data-os-params="current_step=agents" data-os-action="<?php echo OsRouterHelper::build_route_name('wizard', 'load_step'); ?>" class="wizard-finished-editing-trigger latepoint-btn latepoint-btn-lg latepoint-btn-secondary"><?php _e('Annulla', 'latepoint'); ?></button>
					</div>
				<?php } ?>
