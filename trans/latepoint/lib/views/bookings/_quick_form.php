  ...
  <form action="" 
    ...
    <div class="os-form-header">
      <?php if($booking->is_new_record()){ ?>
        <h2><?php _e('Nuovo Appuntamento', 'latepoint'); ?></h2>
      <?php }else{ ?>
    ...
    <div class="os-form-content">
      ...
      <?php 
        ...
        echo '<div class="latepoint-message latepoint-message-error">'.__('Nessun servizio attivo trovato.', 'latepoint').'</div>';
      } ?>
      ...
      <?php if(OsLocationHelper::count_locations(OsAuthHelper::get_logged_in_agent_id()) > 1){ ?>
        <div class="os-row">
          <div class="os-col-12">
            <?php echo OsFormHelper::select_field('booking[location_id]', __('Seleziona la località', 'latepoint'), OsLocationHelper::get_locations_list(), $booking->location_id, ['class' => 'location_id_holder']); ?>
          </div>
        </div>
        <?php
      }else{
      ...
      <div class="os-row">
        <div class="os-col-6">
          <div class="agent-info-w <?php echo ($booking->agent_id) ? 'selected': 'selecting'; ?>">
            <div class="agents-selector-w">
              <div class="os-form-group os-form-select-group os-form-group-transparent">
                <label for=""><?php _e('Personale Scolastico', 'latepoint'); ?></label>
        ...
        <div class="os-col-6">
          <?php echo OsFormHelper::select_field('booking[status]', __('Status', 'latepoint'), OsBookingHelper::get_statuses_list(), $booking->status, array('placeholder' => __('Seleziona Status', 'latepoint'))); ?>
        </div>
      </div>
      <div class="os-row">
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[start_date]', __('Data ', 'latepoint'), $booking->format_start_date(), array('class' => '', 'data-route' => OsRouterHelper::build_route_name('bookings', 'quick_availability'))); ?>
        </div>
        <div class="os-col-6">
          <a href="#" data-route="<?php echo OsRouterHelper::build_route_name('bookings', 'quick_availability'); ?>" class="latepoint-btn latepoint-btn-white open-quick-availability-btn trigger-quick-availability"><i class="latepoint-icon latepoint-icon-calendar"></i><span><?php _e('Mostra Calendario', 'latepoint'); ?></span></a>
        </div>
      </div>
      <div class="os-row">
        <div class="os-col-12">
          <div class="ws-period os-period-transparent">
            <div class="quick-start-time-w">
              <?php echo OsFormHelper::time_field('booking[start_time]', __('Orario di Inizio', 'latepoint'), $booking->start_time, true); ?>
            </div>
            <div class="quick-end-time-w">
              <?php echo OsFormHelper::time_field('booking[end_time]', __('Orario di Fine', 'latepoint'), $booking->end_time, true); ?>
            </div>
          </div>
        </div>
      </div>
      <div class="os-row">
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[buffer_before]', __('Buffer Prima', 'latepoint'), $booking->buffer_before); ?>
        </div>
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[buffer_after]', __('Buffer Dopo', 'latepoint'), $booking->buffer_after); ?>
        </div>
      </div>
      <div class="os-row">
        <div class="os-col-12">
          <?php echo OsFormHelper::textarea_field('booking[customer_comment]', __('Commento lasciato dall\'Utente', 'latepoint'), $booking->customer_comment, ['rows' => 1]); ?>
        </div>
      </div>
    ...
    <div class="os-form-buttons">
      <div class="os-row">
        ...
        <div class="os-col-6">
          <a href="#" class="latepoint-side-panel-close-trigger latepoint-btn latepoint-btn-secondary latepoint-btn-block"><?php _e('Annulla', 'latepoint'); ?></a>
        </div>
      </div>
    </div>
    <?php 
