...
<?php }else{
	echo '<div class="search-no-results">'.__('No Matched Customers found.', 'latepoint').' <a href="'.OsRouterHelper::build_link(OsRouterHelper::build_route_name('customers', 'create')).'">'.__('Aggiungi Utente', 'latepoint').'</a></div>';
} ?>
<?php if($logged_in_admin_user_id){ 
	// This results are only for admins
	?>
	<h3><i class="latepoint-icon latepoint-icon-briefcase"></i><span><?php _e('Personale Scolastico', 'latepoint'); ?></span></h3>
	<?php if($agents){ ?>
		<div class="latepoint-search-results-tiles-w">
	...
	<?php }else{
		echo '<div class="search-no-results">'.__('No Matched Agents found.', 'latepoint').' <a href="'.OsRouterHelper::build_link(OsRouterHelper::build_route_name('agents', 'create')).'">'.__('Aggiungi Personale Scolastico', 'latepoint').'</a></div>';
	} ?>
