...
if($agents){ ?>
	...
	if(!$calendar_only) echo '<div class="calendar-month-agents-w '.$calendar_not_scrollable_class.'" data-route="'.OsRouterHelper::build_route_name('calendars', 'monthly_agents').'">';
		echo '<div class="ma-floated-days-w">';
			echo '<div class="ma-head-info"><span>'.__('Date', 'latepoint').'</span><span>'.__('Personale Scolastico', 'latepoint').'</span></div>';
  ...
  <div class="no-results-w">
    ...
    <h2><?php _e('Nessun Personale Selezionato ', 'latepoint'); ?></h2>
    <a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('agents', 'new_form') ) ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus-square"></i><span><?php _e('Aggiungi un nuovo membro al personale', 'latepoint'); ?></span></a>
  </div>
<?php } ?>
