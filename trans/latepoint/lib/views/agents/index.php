<?php if($agents){ ?>
	<div class="index-agent-boxes">
        ...
        <div class="create-agent-link-i">
          ...
          <div class="add-agent-label"><?php _e('Aggiungi Personale Scolastico', 'latepoint'); ?></div>
        </div>
			</a>
		<?php } ?>
	</div>
<?php }else{ ?>
  <div class="no-results-w">
    ...
    <h2><?php _e('Nessun membro scolastico trovato', 'latepoint'); ?></h2>
    <a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('agents', 'new_form') ) ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus"></i><span><?php _e('Aggiungi il primo membro scolastico', 'latepoint'); ?></span></a>
  </div>
<?php } ?>
