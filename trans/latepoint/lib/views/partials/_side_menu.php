<div class="latepoint-side-menu-w <?php echo (OsAuthHelper::is_admin_logged_in()) ? 'side-menu-'.OsSettingsHelper::get_menu_layout_style() : 'side-menu-compact'; ?>">
	...
	<div class="back-to-wp-link-w">
		<a class="back-to-wp-link" href="<?php echo get_admin_url(); ?>">
			...
			<span><?php _e('Torna al sito web', 'latepoint'); ?></span>
		</a>
	</div>
	<?php } ?>
	<ul class="side-menu">
		...
		foreach(OsMenuHelper::get_side_menu_items() as $menu_item){
			...
			?>
			<li class="<?php if(isset($menu_item['children'])) echo ' has-children'; ?><?php if($is_active) echo ' menu-item-is-active'; ?>" <?php if($is_active) echo 'style="margin-left: 0px;
' . "border-top: 1px solid ". $menu_item['color'] .";" . "border-bottom: 1px solid ". $menu_item['color'] .";" . "border-left: 10px solid ". $menu_item['color'] .";" . '"'; ?>>
				<a href="<?php echo $menu_item['link']; ?>">
					<i style="<?php if(isset($menu_item['image'])) echo 'display: none;' ?>" class="<?php echo $menu_item['icon']; ?>"></i>
					<?php if(isset($menu_item['image'])) echo '<img src="'.$menu_item['image'].' " alt="'.$menu_item['label'].'" width="22" >' ?>

					<span style="color: <?php if($is_active) echo $menu_item['color']; ?>"><?php echo $menu_item['label']; ?></span>
				</a>
		...
		<?php if(OsAuthHelper::is_admin_logged_in()){ ?>
			<li class="back-to-wp-item">
				<a href="<?php echo get_admin_url(); ?>"><i class="latepoint-icon latepoint-icon-wordpress"></i><span><?php _e('Torna al sito web', 'latepoint'); ?></span></a>
				<ul class="side-sub-menu only-menu-header"><li class="side-sub-menu-header"><?php _e('Torna al sito web', 'latepoint'); ?></li></ul>
			</li>
		<?php } ?>
	</ul>
</div>
