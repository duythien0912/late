<div class="os-form-w">
      ...
      <div class="white-box-content">
				<div class="os-row">
				  <div class="os-col-lg-12">
				    <?php echo OsFormHelper::media_uploader_field('customer[avatar_image_id]', 0, __('Seleziona il tuo Avatar', 'latepoint'), __('Remove Avatar', 'latepoint'), $customer->avatar_image_id); ?>
				  </div>
				</div>
				<div class="os-row">
					<div class="os-col-6">
				    <?php echo OsFormHelper::text_field('customer[first_name]', __('Nome ', 'latepoint'), $customer->first_name); ?>
					</div>
					<div class="os-col-6">
				    <?php echo OsFormHelper::text_field('customer[last_name]', __('Cognome', 'latepoint'), $customer->last_name); ?>
					</div>
				</div>
				<div class="os-row">
					<div class="os-col-lg-6">
				    <?php echo OsFormHelper::text_field('customer[email]', __('Indirizzo Email', 'latepoint'), $customer->email); ?>
					</div>
					<div class="os-col-lg-6">
				    <?php echo OsFormHelper::text_field('customer[phone]', __('Il tuo Numero di Telefono', 'latepoint'), $customer->phone, array('class' => 'os-mask-phone')); ?>
					</div>
				</div>
				<div class="os-row">
				  <div class="os-col-lg-12">
				    <?php echo OsFormHelper::textarea_field('customer[notes]', __(`Note dell'utente`, 'latepoint'), $customer->notes); ?>
				  </div>
				</div>
				<div class="os-row">
				  <div class="os-col-lg-12">
				    <?php echo OsFormHelper::textarea_field('customer[admin_notes]', __(`Note dell' amministrazione (visibili solo all'amministrazione scolastica)`, 'latepoint'), $customer->admin_notes); ?>
				  </div>
				</div>
			</div>
		</div>
    ...
    <?php 
      if($customer->is_new_record()){
        echo OsFormHelper::button('submit', __('Salva', 'latepoint'), 'submit', ['class' => 'latepoint-btn']); 
      }else{
		...
		if($customer->bookings){
		  ...
		  <div class="no-results-w">
		    ...
		    <a <?php echo OsBookingHelper::quick_booking_btn_html(false, array('customer_id'=> $customer->id)); ?> href="#" class="latepoint-btn">
		      ...
		      <span><?php _e('Crea un appuntamento', 'latepoint'); ?></span>
		    </a>
		  </div>
			<?php
		} ?>
